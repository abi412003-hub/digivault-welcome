...
const OrganizationRegistration = () => {
  ...
  const updateProfileCascade = (field: keyof OrganizationProfile, value: string) => {
    const cascadeFields: Record<string, (keyof OrganizationProfile)[]> = {
          ...
          <section>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="date"
                  value={profile.dateOfEstablishment ? format(profile.dateOfEstablishment, "yyyy-MM-dd") : ""}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val) {
                      const d = new Date(val + "T00:00:00");
                      if (!isNaN(d.getTime()) && d <= new Date()) {
                        updateProfile("dateOfEstablishment", d);
                      }
                    } else {
                      updateProfile("dateOfEstablishment", undefined);
                    }
                  }}
                  max={format(new Date(), "yyyy-MM-dd")}
                  className="w-full"
